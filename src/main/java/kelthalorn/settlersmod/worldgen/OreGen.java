package kelthalorn.settlersmod.worldgen;
...
public class OreGen implements IWorldGenerator {
	...
	public OreGen() {
		tin_overworld = new WorldGenMinable(
				ModBlocks.tinOre.getDefaultState().withProperty(BlockTinOre.TYPE, EnumHandler.OreType.OVERWORLD), 8); //Here we are saying that our ore spawns in veins of up to 8
		tin_nether = new WorldGenMinable(
				ModBlocks.tinOre.getDefaultState().withProperty(BlockTinOre.TYPE, EnumHandler.OreType.NETHER), 8,
				new NetherGenPredicate()); //Uses the nether predicate so that when it is being generated it replaces the netherrack
		tin_end = new WorldGenMinable(ModBlocks.tinOre.getDefaultState().withProperty(BlockTinOre.TYPE, EnumHandler.OreType.END), 8,
				new EndGenPredicate());
	}
